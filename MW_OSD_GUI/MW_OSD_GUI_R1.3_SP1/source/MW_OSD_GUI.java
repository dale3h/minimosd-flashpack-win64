...
// Layout editor
  ...
  if (toggleModeItems[9].getValue()>0)
    hudid = PApplet.parseInt(confItem[GetSetting("S_HUDOSDSW")].value());
  else
    hudid = PApplet.parseInt(confItem[GetSetting("S_HUD")].value()); 
...
//################################################################################################################################################################################ 
 ...
 for(int i = 0; i < (hudoptions); i++){
   ...
   ConfigLayout[0][i]=CONFIGHUD[PApplet.parseInt(confItem[GetSetting("S_HUD")].value())][i];
   ConfigLayout[1][i]=CONFIGHUD[PApplet.parseInt(confItem[GetSetting("S_HUDOSDSW")].value())][i];
...
public void bLUP() {
  int i = 0;
  if (toggleModeItems[9].getValue()>0)
    i = PApplet.parseInt(confItem[GetSetting("S_HUDOSDSW")].value());
  else
    i = PApplet.parseInt(confItem[GetSetting("S_HUD")].value()); 
...
public void WRITEinit(){
  ...
  readerror=1;  
  CheckCallSign();
  S16_AMPMAX = PApplet.parseInt(confItem[GetSetting("S_AMPDIVIDERRATIO")].value());
  ...
  for(int i = 0; i < CONFIGITEMS; i++){
    readcheck[i]=PApplet.parseInt(confItem[i].value());
  }
  ...
  for (int txTimes = 0; txTimes<1; txTimes++) {
    ...
    for(int i = 0; i < CONFIGITEMS; i++){
      if(i == GetSetting("S_VOLTAGEMIN")){
        serialize8(PApplet.parseInt(confItem[i].value()*10));//preserve decimal
      }
      else if(i == GetSetting("S_GPSTZ")){
        serialize8(PApplet.parseInt(confItem[i].value()*10));//preserve decimal, maybe can go elsewhere - haydent
      }
...
public void ShowMapMode(){
  int mi;
  if (toggleModeItems[9].getValue()>0)
    mi = PApplet.parseInt(confItem[GetSetting("S_HUDOSDSW")].value());
  else
    mi = PApplet.parseInt(confItem[GetSetting("S_HUD")].value()); 
